...
def get_emcee_run(cli):
    ...
    if cli.emcee_old_save:
        ...
        sys.stdout.flush()
        chains_posterior = backend.get_chain(discard=cli.nburnin, flat=False, thin=cli.use_thin)
        ...
        sys.stdout.flush()
        fitting_posterior = backend.get_chain(discard=cli.nburnin, flat=True, thin=cli.use_thin)
        ...
        sys.stdout.flush()
        lnprobability_posterior = backend.get_log_prob(discard=cli.nburnin, flat=False, thin=cli.use_thin)
        anc.print_both("... loading thinned flat posterior log_prob ...")
        sys.stdout.flush()
        lnprob_posterior = backend.get_log_prob(discard=cli.nburnin, flat=True, thin=cli.use_thin)
        sys.stdout.flush()
...
class AnalysisTRADES:
    def __init__(self, cli):
        ...
        sim = pytrades.TRADES(cli.full_path, seed=cli.seed, m_type=cli.m_type)
        sim.init_trades()
        sys.stdout.flush()
        ...
        anc.print_both("sim.m_factor = {} sim.mass_unit = {}".format(sim.m_factor, sim.mass_unit))
        (
        ...
        anc.print_both("mass_conv_factor = {} mass_unit = {}".format(self.mass_conv_factor, self.mass_unit))

        sys.stdout.flush()
        stellar_mass_normal = np.random.normal(
            loc=self.star[0, 0], scale=self.star[0, 1], size=self.npost
        )
        self.mass_conv_factor_post = (self.mass_conv_factor/self.star[0, 0]) * stellar_mass_normal

        stellar_radius_normal = np.random.normal(
            loc=self.star[1, 0], scale=self.star[1, 1], size=self.npost
        )
        self.radius_conv_factor_post = (self.radius_conv_factor/self.star[1,0]) * stellar_radius_normal
    ...
    def save_pso_parameters(self):
        ...
        if pso:
            ...
            _, pso_phy, _ = anc.compute_physical_parameters(
                self.sim.n_bodies,
                pso_fit,
                self.fitting_names,
    ...
    def save_de_parameters(self):
        ...
        if de:
            ...
            _, de_phy, _ = anc.compute_physical_parameters(
                self.sim.n_bodies,
                de_fit,
                self.fitting_names,
                self.sim.system_parameters,
    ...
    def save_map_parameters(self):
        ...
        _, map_phy, _ = anc.compute_physical_parameters(
            self.sim.n_bodies,
            map_fit,
            self.fitting_names,
            self.sim.system_parameters,
    ...
    def save_mode_parameters(self):
        ...
        median = np.percentile(self.fitting_posterior, 50.0, axis=0,
            # 
        )
        ...
        if np.any(np.isnan(mode_fit)):
            ...
            if neg_values:
                ...
                median = np.percentile(
                    self.physical_posterior, 50.0, axis=0,
                    #  
                )
    ...
    def save_from_file_parameters(self):
        ...
        if self.cli.from_file is not None:
            ...
            _, adhoc_phy, _ = anc.compute_physical_parameters(
                self.sim.n_bodies,
                adhoc_fit,
                self.fitting_names,
                self.sim.system_parameters,
    ...
    def save_mle_parameters(self):
        # ==============================================================================
        # MLE == maximum loglikelihood of posterior within HDI ID == 668
        # ==============================================================================
        id_sim = 668
        sim_name = "mle"
        par_desc = "mle of posterior"
        anc.print_both("\nPARAMETERS: {} ==> {}".format(par_desc, id_sim))

        mle_fit, _, mle_idx = anc.select_maxlglhd_with_hdi(
            self.fitting_posterior,
            self.ci_fitted.T,
            self.lnprob_posterior,
            return_idmax=True,
        )
        # mle_phy = self.physical_posterior[mle_idx, :]
        _, mle_phy, _ = anc.compute_physical_parameters(
            self.sim.n_bodies,
            mle_fit,
            self.fitting_names,
            self.sim.system_parameters,
...
def run_analysis(cli):
    ...
    anc.print_both("shape of chains_posterior = {}".format(np.shape(analysis.chains_posterior)))
    anc.print_both(
        ...
        if len(cli.ocs) > 0:
            for coc in cli.ocs:
                ...
                for idbody, flist in file_list.items():
                    ...
                    fig = poc.plot_oc_T41(coc, flist, planet_name=pl_name, samples=samples_oc, save_plot=True, show_plot=False)
                    plt.close(fig)
