...
if cli.emcee_previous is not None:
    ...
    if pytrades.lmon == 1:
        ...
        def minimize_func(fit_par):

            lnL = lnprob_sq(fit_par, parameter_names)
            to_min = -2.0*lnL
        ...
        chi_square_opt, reduced_chi_square_opt, \
            lgllhd_opt, lnc_opt, bic_opt, check_opt = pytrades.write_summary_files(
            1, fit_trades_opt
        )
        anc.print_both("",of_run)
