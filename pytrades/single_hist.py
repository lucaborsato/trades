...
def main():
  ...
  step_min = 0
  step_max = completed_steps
  delta_step = 400
  id_fit = 7 # 8th parameter - 1

  sel_min = np.arange(step_min,completed_steps,delta_step)
  sel_max = sel_min + delta_step
  ...
  par_full_posterior = flatchain_posterior_0[:,id_fit]

  par_min = np.min(par_full_posterior)
  par_max = np.max(par_full_posterior)
  ...
  for isel in range(0, sel_min.shape[0]):
    ...
    par_sel = par_full_posterior[sel_min[isel]:sel_max[isel]]
    k = np.ceil(2. * par_sel.shape[0]**(1./3.)).astype(int)
    if(k>50): k=50
    print 'selected slide of the chain'
    axHist = plt.subplot2grid((n_sel, 1), (isel,0))
    (counts, bins_val, patches) = axHist.hist(par_sel, bins=k, range=(par_sel.min(), par_sel.max()), orientation='vertical', normed=True, stacked=True, histtype='stepfilled', color='darkgrey', edgecolor='lightgray', align='mid', label='%s [steps %d : %d]' %(kel_legends[0],sel_min[isel], sel_max[isel]))
    print 'plotted histogram'
    xpdf = scipy_norm.pdf(par_sel, loc = par_sel.mean(), scale = par_sel.std())
    idx = np.argsort(par_sel)
    axHist.plot(par_sel[idx], xpdf[idx], color='black', marker='None', ls='-.', lw=1.5, label='pdf')
    ...
    #axHist.set_xlim([par_min, par_max])
  ...
  emcee_fig_file = os.path.join(emcee_plots, 'sel_hist_' + parameter_names_emcee[id_fit].strip() + '.png')
