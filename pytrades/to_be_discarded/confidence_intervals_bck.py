...
def save_models_from_parameters(
    fit_par_in, names_par_in, NB, smp_name, smp_h5, trades_type=False, out=None
):

    gr = smp_h5.create_group(smp_name)

    # FROM TRADES v2.18.0 the program fit sqrt(e)cos/sin(w), so trades_type=False is alway ok
    # set it manually only for backward compatibility
    if trades_type:
        ...
        # convert trades par to python fit
    ...
    (
        ...
        rv_nmax,
        stats_rv,
    ) = pytrades.fit_par_to_ttra_rv(trades_fit_par, nt0_full, nrv_nmax)
...
def save_ttra_and_rv_from_samples(
    samples_fit_par, names_par, NB, n_samples, smp_h5, trades_type=False, out=None
):

    anc.print_both("=" * 40, out)
    anc.print_both(
        "= BEGIN = Getting Transit Times and RV from samples ...", out)

    # n_samples, nfit = np.shape(samples_fit_par)
    n_samples, _ = np.shape(samples_fit_par)

    # Pephem = pytrades.pephem
    # print('NB = ',NB,' np.shape(Pephem) = ',np.shape(Pephem), ' Pephem = ', Pephem)
    # try:
    #   nt0_full, nrv_nmax = pytrades.get_max_nt0_nrv(Pephem,NB)
    # except:
    #   jP = (np.arange(2,NB+1,1)-2)*8 + 5 - 1
    #   print('jP = ',jP)
    #   Pephem = np.zeros((NB))
    #   Pephem[1:] = pytrades.system_parameters[jP]
    #   print('Pephem = ',Pephem)
    #   nt0_full, nrv_nmax = pytrades.get_max_nt0_nrv(Pephem,NB)
    ...
    anc.print_both(
        "= END = Getting Transit Times and RV from samples ...", out)
    anc.print_both("=" * 40, out)
    sys.stdout.flush()
...
def main():
    ...
    # init trades
    pytrades.initialize_trades(os.path.join(cli.full_path, ""), "", 1)

    # nfit, NB, bodies_file, id_fit, id_all, nfit_list, cols_list, case_list = anc.get_fitted(cli.full_path)
    nfit, NB, _, id_fit, _, _, cols_list, case_list = anc.get_fitted(
        cli.full_path)
    ndata = pytrades.ndata
    nfree = pytrades.nfree
    dof = pytrades.dof
    # print("ndata {} nfree {} dof {}".format(
    #     ndata,nfree,dof
    # ))
    ...
    emcee_file, _, _ = anc.get_emcee_file_and_best(
    ...
    chains_T, flatchain_posterior_0, lnprob_burnin, _ = anc.thin_the_chains(
        cli.use_thin,
        nburnin,
        nruns,
        nruns_sel,
    ...
    for name in names_par:
        if "lambda" in name:
            i_fit = names_par.index(name)
            flatchain_posterior_0[:, i_fit] = flatchain_posterior_0[:,i_fit]%360.0

    # computes mass conversion factor
    # m_factor = anc.mass_conversion_factor(cli.m_type)
    MR_star = pytrades.mr_star
    # m_factor_0, mass_unit = anc.mass_type_factor(
    #     Ms=1.0, mtype=cli.m_type, mscale=False)
    m_factor_0, mass_unit, r_factor_0, radius_unit = anc.mass_radius_type_factor(
        mtype=cli.m_type
    )
    # mass conversion
    Ms_gaussian = np.abs(
        MR_star[0, 0]
        + np.random.normal(0.0, 1.0, size=(np.shape(flatchain_posterior_0)[0]))
        * MR_star[0, 1]
    )  # if exists an error on the mass, it creates a Normal distribution for the values and use it to re-scale mp/Ms to mp.
    m_factor_boot = (
        m_factor_0 * Ms_gaussian
    )  # given the factor from Msun to mass_unit it multiply it by the Normal Mstar.
    ...
    r_factor_boot = (
        r_factor_0 * Rs_gaussian
    )
    r_factor = r_factor_0 * MR_star[1, 0]
    ...
    p_h5f.create_dataset(
        "parameter_names", data=anc.encode_list(names_par), dtype="S15"
    )
    ...
    for ifit in range(0, nfit):
        if "Ms" in names_par[ifit]:
            flatchain_posterior[:, ifit] = m_factor_0 * \
                flatchain_posterior[:, ifit]
    ...
    def get_intervals(
        ...
        out_folder = os.path.join(
            os.path.join(full_path, "{:04d}_sim".format(id_sim)), ""
        )
        ...
        smp_name = "{:04d}".format(id_sim)
        smp_h5 = h5py.File(
            os.path.join(out_folder, "{}_models.hdf5".format(smp_name)), "w"
        )

        # -- START DEBUG
        # anc.print_both("# -- START DEBUG", out)
        # check parameters within boundaries
        # nfit = len(parameters_in)
        # temp_names = pytrades.get_parameter_names(nfit, pytrades.str_len)
        # trades_names = anc.convert_fortran_charray2python_strararray(temp_names)
        # trades_par =  anc.sqrte_to_e_fitting(parameters_in, names_par_in)
        # trades_minmax = pytrades.parameters_minmax
        # tra_min = trades_minmax[:,0]
        # tra_max = trades_minmax[:,1]
        # anc.print_both("# trades type", out)
        # for n, p, m, mm in zip(trades_names,trades_par,tra_min,tra_max):
        #     anc.print_both("{:12s} {:12.5f} {:12.5f} {:12.5f}".format(n,p,m,mm), out)
        # parameters_minmax = anc.e_to_sqrte_boundaries(trades_minmax, trades_names)
        # par_min = parameters_minmax[:,0]
        # par_max = parameters_minmax[:,1]
        # anc.print_both("# emcee type", out)
        # for n, p, m, mm in zip(names_par_in,parameters_in,par_min,par_max):
        #     anc.print_both("{:12s} {:12.5f} {:12.5f} {:12.5f}".format(n,p,m,mm), out)
        # anc.print_both("# -- END DEBUG")
        # -- END DEBUG
        ...
        # FROM TRADES v2.18.0 NOT NEEDED ANYMORE
        # parameters_trades = anc.sqrte_to_e_fitting(
        #     parameters_in, names_par_in
        # )  # emcee to trades
        ...
        chi_square, reduced_chi_square, lgllhd, lnprior, ln_const, bic, check = \
            pytrades.write_summary_files(
                id_sim,
                # parameters_trades # OLD
                parameters # FROM TRADES v2.18.0
            )
        gr = smp_h5[smp_name]
        ...
        sigma_par = anc.compute_sigma_hdi(
            flatchain_posterior, parameters)  # uses HDI
        ...
        der_posterior_T = anc.derived_posterior_check(
            names_derived, der_posterior)

        par_type = ""
        descr = ""
        if str(derived_type).strip().lower() == "median":
            # MEDIAN PARAMETERS ID == 1050
            derived_par = np.percentile(
                der_posterior_T, 50.0, axis=0, interpolation="midpoint"
            )
            par_type = "MEDIAN:"
            ...
            if id_sim == 0:
                ...
                par_type = "BEST PSO PARAMETERS"
                descr = "Best parameter set from PSO analysis"
            elif id_sim == 6:
                par_type = "BEST DE PARAMETERS"
                descr = "Best parameter set from DE analysis"
            else:
                par_type = "AD HOC"
        ...
        if s_h5f is not None:
            ...
            s_h5f.create_dataset(
                "parameters/{:s}/fitted/names".format(s_id_sim),
                data=anc.encode_list(names_par),
                dtype="S10",
                compression="gzip",
            )
            s_h5f.create_dataset(
                "parameters/{:s}/fitted/units".format(s_id_sim),
                data=anc.encode_list(units_par),
                dtype="S15",
                compression="gzip",
            )
            s_h5f.create_dataset(
            ...
            s_h5f.create_dataset(
                "parameters/{:s}/derived/names".format(s_id_sim),
                data=anc.encode_list(names_derived),
                dtype="S10",
                compression="gzip",
            )
            s_h5f.create_dataset(
                "parameters/{:s}/derived/units".format(s_id_sim),
            ...
            s_h5f["parameters/{:s}".format(s_id_sim)].attrs[
                "info"
            ] = "{} ==> {}".format(s_id_sim, par_type)
            s_h5f["parameters/{:s}".format(s_id_sim)
                  ].attrs["chi_square"] = chi_square
            s_h5f["parameters/{:s}".format(s_id_sim)
                  ].attrs["reduced_chi_square"] = reduced_chi_square
            s_h5f["parameters/{:s}".format(s_id_sim)].attrs["lgllhd"] = lgllhd
            s_h5f["parameters/{:s}".format(s_id_sim)
                  ].attrs["ln_const"] = ln_const
            s_h5f["parameters/{:s}".format(s_id_sim)].attrs["bic"] = bic
        ...
        anc.print_parameters(
            top_header, header, names_par, units_par, parameters, sigma_par, out
        )
    ...
    original_fit_parameters = (
        pytrades.fitting_parameters
    ...
    # original_parameters = anc.e_to_sqrte_fitting(
    #     original_fit_parameters, trades_names)
    # NEW: TRADES v2.18.0 fits (sqrecosw, sqrtesinw)
    original_parameters = original_fit_parameters
    ...
    # pso_file = glob.glob(pso_pattern)[0]
    pso = os.path.exists(pso_file) and os.path.isfile(pso_file)
    if pso:
        fpso = h5py.File(pso_file, 'r')
        pso_parameters = fpso["pso_parameters"][...].copy()
        fpso.close()
        _ = get_intervals(
            cli.full_path,
            4,
    ...
    if de:
        fde = h5py.File(de_file, 'r')
        de_parameters = fde["de_parameters"][...].copy()
        fde.close()
        _ = get_intervals(
            cli.full_path,
            6,
    ...
    # ==============================================================================
    _, max_lnprob_parameters, _, _ = anc.get_maxlnprob_parameters(
        lnprob_burnin, chains_T, flatchain_posterior_0
    )
    # max_id1, max_id2 = anc.get_max_indices(lnprob_burnin)
    _, names_derived, der_posterior = get_intervals(
        cli.full_path,
        2050,
        names_par,
    ...
    s_h5f.create_dataset(
        "confidence_intervals/derived/names",
        data=anc.encode_list(names_derived),
        dtype="S10",
        compression="gzip",
    )
    s_h5f.create_dataset(
        "confidence_intervals/derived/units",
        data=anc.encode_list(units_der),
        dtype="S15",
        compression="gzip",
    )
    ...
    if cli.n_samples > 0:
        anc.print_both(
            " Selecting %d samples from the posterior ..." % (cli.n_samples))
        sys.stdout.flush()
        ...
        samples_fit_par[
            0, :
        ] = median_parameters  # first sample as the median of the posterior
        ...
        save_ttra_and_rv_from_samples(
            samples_fit_par, names_par, NB, cli.n_samples, smp_h5,
            trades_type=False
        )
        smp_h5.close()
        anc.print_both(" ... done")
    ...
    mode_parameters = 3 * median_parameters - \
        2 * np.mean(flatchain_posterior_0, axis=0)
    ...
    if sample_parameters is not None:
        ...
        s_h5f["parameters/{:04d}".format(666)
              ].attrs["par_selection"] = name_par
        if name_excluded is not None:
            s_h5f["parameters/{:04d}".format(666)
                  ].attrs["par_excluded"] = name_excluded
    else:
    ...
    # ==============================================================================
    name_par, name_excluded = anc.get_sample_list(cli.sample_str, names_par)
    sample3_parameters, _ = anc.select_maxlglhd_with_hdi(
        flatchain_posterior_0,
        ci_fitted.T,
        lnprob_burnin.T,
    )
