...
anc.print_both(' Total N_RV = {:d} for {:d} set(s)'.format(
    n_rv, n_set_rv
    ), 
    of_run
)
anc.print_both(' Total N_T0 = {:d} for {:d} out of {:d} planet(s)'.format(
...
for iter_global in range(0, n_global):
    ...
    if (nsave != False):
        ...
        f_hdf5.create_dataset(
            'boundaries', data=parameters_minmax, dtype=np.float64)
        f_hdf5.create_dataset(
            'chains', (nwalkers, nruns, nfit), dtype=np.float64)
        ...
        f_hdf5.create_dataset(
            'lnprobability', (nwalkers, nruns), dtype=np.float64)
        # f_hdf5['lnprobability'].attrs['ln_err_const'] = ln_err_const
        f_hdf5.create_dataset('acceptance_fraction',
                              data=np.zeros((nfit)), dtype=np.float64)
        f_hdf5.create_dataset(
            'autocor_time', data=np.zeros((nfit)), dtype=np.float64)
        f_hdf5.close()
        ...
        anc.print_both('', of_run)
        anc.print_both('...done with saving temporary total shape = {:s}'.format(
