...
def pytransit_select_index(i, npop):
    v1, v2, v3 = i, i, i
    while v1 == i:
        v1 = randint(npop)
    while (v2 == i) or (v2 == v1):
...
def evolve_population(pop, pop2, f, c):
    ...
    for i in range(npop):
        # --- Vector selection ---
        v1, v2, v3 = trades_select_index(i, idx)
        # v1, v2, v3 = pytransit_select_index(i, npop)
...
class DiffEvol(object):
    ...
    def _eval_sfun(self, ngen=1):
        ...
        pc = [popc[ipop, :] for ipop in range(self.n_pop)]
        fitc[:] = self.m * array(list(self.map(self.minfun, pc)))
        # print("==== Initialized PyDE popc => fitc", flush=True)
        ...
        for igen in range(ngen):
            ...
            # print("{}: evolve_population".format(l))
            popt = evolve_population(popc, popt, f, c)
            # print("{}: fitt with map".format(l))
            fitt[:] = self.m * array(list(self.map(self.minfun, popt)))

            # print("{}: select fitt < fitc".format(l))
            msk = fitt < fitc
            popc[msk, :] = popt[msk, :]
            fitc[msk] = fitt[msk]

            # print("{}: minidx of fitc".format(l))
