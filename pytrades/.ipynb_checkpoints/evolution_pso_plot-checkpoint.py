...
class PSOEvolution:
    ...
    def __init__(self, pso_path):
        ...
        all_pattern = os.path.join(
            pso_path, "*_allpso_*par.out"
        )
        ...
        best_pattern = os.path.join(
            pso_path, "*_bestpso_*par.out"
        )
    ...
    def ax_scatter_plot(self, fig, ax, name1, name2, scale_name="pso_fitness", colorbar=True, thin_iter=1):
    ...
    def scatter_plot(self, name1, name2, scale_name="pso_fitness", colorbar=True, thin_iter=1):
        
        fig = plt.figure()
        nrow, ncol = 1, 1        
        irow, icol = 0, 0
        ax = plt.subplot2grid((nrow, ncol), (irow, icol))
        self.ax_scatter_plot(fig, ax, name1, name2, scale_name=scale_name, colorbar=colorbar, thin_iter=thin_iter)
        plt.show()
        plt.close(fig)
        
        return
    
    def threepanel_scatter_plot(self,
      ...
      thin_iter=1
        ...
        self.ax_scatter_plot(fig, ax1, three_pairs[0][0], three_pairs[0][1], scale_name=scale_name, colorbar=False, thin_iter=thin_iter)
        
        # irow, icol = 0, 1
        # ax2 = plt.subplot2grid((nrow, ncol), (irow, icol))
        l += w+dl
        ax2 = fig.add_axes([l,b,w,h])
        self.ax_scatter_plot(fig, ax2, three_pairs[1][0], three_pairs[1][1], scale_name=scale_name, colorbar=False, thin_iter=thin_iter)
        
        # irow, icol = 0, 2
        # ax3 = plt.subplot2grid((nrow, ncol), (irow, icol))
        l += w+dl
        ax3 = fig.add_axes([l,b,w,h])
        self.ax_scatter_plot(fig, ax3, three_pairs[2][0], three_pairs[2][1], scale_name=scale_name, colorbar=True, thin_iter=thin_iter)
    ...
    def adhoc_scatter_plot(self, scale_name="pso_fitness", thin_iter=1):

        self.threepanel_scatter_plot(scale_name=scale_name, thin_iter=thin_iter)
    ...
    def histogram_plot(self, n_planets, label_type="mass"):
        ...
        for i_body in range(0,nc):
            ...
            plt.hist(self.all_pso[k][sel_k], bins=100, color=color_r, histtype='step',label="{} sel lnL".format(k), alpha=0.8)
