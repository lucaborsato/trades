...
def run_simulation(ii, full_fitting_parameters, full_id, full_fitness, full_check):
  set_parameters = full_fitting_parameters[ii]
...
if __name__ == '__main__':
  ...
  print_both(' INITIALISED GRID', of_run)
  
  
  # prepare parallel stuff
  folder = tempfile.mkdtemp()
  id_tempfile = os.path.join(folder, 'id_grid')
  fitness_tempfile = os.path.join(folder, 'fitness_grid')
  check_tempfile = os.path.join(folder, 'check_grid')
  
  try:
    full_id = np.memmap(id_tempfile, dtype=np.int, shape=n_grid, mode='w+')
    full_fitness = np.memmap(id_tempfile, dtype=np.int, shape=n_grid, mode='w+')
    full_check = np.memmap(id_tempfile, dtype=np.int, shape=n_grid, mode='w+')
    ...
    try:
      ...
      print_both("Failed to delete: %s" %(folder), of_run)

  print_both(' GRID COMPLETED', of_run)


  #full_id, full_fitness, full_check = get_sorted_output(output_queue)
