...
def compute_initial_walkers(lnprob, nfit, nwalkers, fitting_parameters, parameters_minmax, parameter_names, delta_sigma, of_run):
...
for iter_global in range(0,n_global):
  ...
  if (cli.pso_type == 'run'):
    ...
    if(float(cli.delta_sigma) <= 0.0):
      p0 = pso_to_emcee(
      nfit, nwalkers,
      population, parameter_names,
      sqrt_par=False
      )
    else:
      p0 = compute_initial_walkers(lnprob, nfit, nwalkers, 
        pso_parameters, parameters_minmax, parameter_names, 
        cli.delta_sigma, 
        of_run
      )
