...
def get_args():
  ...
  parser.add_argument('-n', '--n', '-name','--name',  
                      action='store', dest='name', default='planet X', 
                      help='Name of simulation appearing in the title plot')
 
  parser.add_argument('-u', '--u', '-unit','--unit',  
                      action='store', dest='ocunit', default='min', 
...
def main():
  ...
  if(cli.name is None):
    ...
    plt.title('%s (O-C)' %(cli.name, ), loc='left',
              fontsize=tsize
             )
