...
def get_args():
  ...
  parser.add_argument('-k', '--K', '--K-rv',
    action='store', dest='K_rv', default=None,
  ...
  parser.add_argument('-id', '--id-perturber', 
    action='store', dest='perturber_id', default=3, type=int, 
    help='Id of the perturber body: star is 1, so it has to be from 2 to Number of bodies. Default is 3.'
    )
  
  parser.add_argument('-P-tra', '--P-tra', '-P-transiting', '--P-transiting',
    action='store', dest='period_transiting', default=None, 
...
def set_names_parameters(id_body, mr_unit, of_log):
  ...
  names_lst = ['{0:s}{1:s}'.format(base_names_lst[i], body_num) for i in range(0, n_names)]
  temp_lst = ['{0:s} [{1:s}]'.format(names_lst[i], units[i].strip()) for i in range(0, n_names)]
...
def simulation_and_ttv_analysis(i_grid, transit_id, perturber_id, n_bodies, \
    perturber_par, t_start, t_end, err_T0, nT0_sel, names_lst, units, mr_type,\
    nMC=1000, of_log=None):
  ...
  try:
    ...
    print(' | PID {0:d} || TID {1:d} sim {2:d}: error setting grip par'
  ...
  try:
    ...
    ttra_all, nttra_all, sel_pre, nttra_pre, sel_cheops, nttra_cheops = \
      select_and_sort_ttra(ttra_full, id_ttra_full, stats_ttra.astype(bool), id_transiting=transit_id,
                           t_min_sel=t_start, t_max_sel=t_end, of_log=of_log
                           )
  ...
  if(ttra_all is not None):
    ...
    print(' | PID {0:d} || TID {1:d} sim {2:d} shape(T0_eT0): {3:}'.format(pid, tid, sim_num, np.shape(T0_eT0)))
...
def run_the_simulation(sim_args):
  ...
  results_sim = simulation_and_ttv_analysis(i_grid, transit_id, perturber_id, n_bodies,
                perturber_par, t_start, t_end, err_T0, nT0_sel, names_lst, units, mr_type,
                nMC, of_log=None
                )
...
def plot_rms(out_folder, body_id, grid_par, p_names, pl_lab, err_T0_d, rms, above, rms_head,\
             rms_max_in = None, K_rv=None, rms_rv=None, of_log=None):
  ...
  if(K_rv is not None and rms_rv is not None):
    ...
    if(np.sum(np.asarray(sel_rv, dtype=np.int)) == 0):
      anc.print_both(' There are no simulations with rms(RV) within 0.0 and {0:.2f} m/s.\n No plot to do.'
                     .format(K_rv), of_log
                     )
      return None
  ...
  anc.print_both(' | plotting {0:s} histogram witn nbins (all) = {1:d}'.format(rms_head, kbins), of_log)
  ...
  anc.print_both(' | plotting {0:s} histogram witn nbins (above) = {1:d}'
                 .format(rms_head, kbins), of_log)
  ...
  for iy in range(0,npar):
    ...
    ax.scatter(rms[sel_rms_a], yy[sel_rms_a],
      marker='o', s=25, c='None', edgecolors='C0', lw=1.1, alpha=1.0, zorder=8,
      label='observable'
      )
    ax.scatter(rms[sel_rms_a], yy[sel_rms_a],
      c=sims[sel_rms_a], cmap=color_map, marker='o', s=24, lw=0, alpha=1.0, zorder=8)
    ...
    ax.scatter(rms[sel_rms_b], yy[sel_rms_b], c=sims[sel_rms_b],
      cmap=color_map, marker='o', s=25, lw=0, alpha=0.25, zorder=8)
...
def plot_summary(out_folder, hdf5_file, body_id=3, mr_type='e', rms_max_in = None, K_rv = None, of_log=None):
  ...
  pl_lab = [r'${0:s}_\textrm{{1:s}}$ {2:s}'.format(l_names[i_n], letters[body_id-1], units[i_n].replace('[]',' ').strip())
  ...
  for irms in range(0, nrms):
    fig_file = plot_rms(plot_folder, body_id, grid_par[:,id_noRtau],
               np.array(p_names)[id_noRtau], np.array(pl_lab)[id_noRtau],
               err_T0_d, rms_grid[:,irms], above_threshold[:,irms], rms_header[irms], rms_max_in,
               K_rv, rms_rv, of_log
...
def compute_grid_to_freq_obs(grid_par, rms_all, lower_rms=0., upper_rms=15.0/1440.0, K_rv=None, rms_rv=None, of_log=None):
...
def plot_m_vs_p_vs_freq_obs(plot_file, mass, period, freq_obs, freq_rv, n_label = 'all 3 5 6 10 15'.split()):
  ...
  for iplt in range(0, nplt):
      ...
      for i_p in range(0, n_p):
        for i_m in range(0, n_m):
          if(freq_rv[ipm] > 0.):
            ...
            mesh_rv[i_p, i_m] = 1.0
          ipm += 1
      ax.contourf(mesh_p, mesh_m, mesh_rv.T, colors='gray', 
                           #cmap=cm.gray, 
    ...
    ax.axhline(np.log10(1.0) , color='lightgray', ls='-', lw=0.8, alpha=0.66, zorder=6)
    ax.axhline(np.log10(5.0) , color='lightgray', ls='-', lw=0.8, alpha=0.66, zorder=6)
    ax.axhline(np.log10(10.0), color='lightgray', ls='-', lw=0.8, alpha=0.66, zorder=6)
    ...
    ax.set_xlabel(r'$\log P [\textrm{{days}}] , N_{{T0}} = {:s}$'.format(n_label[iplt]))
...
def read_and_plot_p_vs_m_vs_freq(out_folder, hdf5_file, rms_min = None, rms_max = 15.0/1440.0, K_rv = None, of_log=None):
  ...
  mass, period, freq_obs, freq_rv = compute_grid_to_freq_obs(grid_par, rms_grid,
                                    lower_rms= rms_lower, upper_rms=rms_max,
                                    K_rv=K_rv, rms_rv=rms_rv,
                                    of_log=of_log
                                    )
  ...
  plt_file_mean = os.path.join(out_folder, 'p_vs_m_rms_{0:s}_max{1:.2f}min'.format('mean',   rms_max*1440.0))
  plt_file_med  = os.path.join(out_folder, 'p_vs_m_rms_{0:s}_max{1:.2f}min'.format('median', rms_max*1440.0))
  if(K_rv is not None and rms_rv is not None):
...
def save_chi2r_rms_analysis(f_h5, chi2r_oc_grid, rms_oc_grid,\
  above_threshold, nT0_sel, rms_rv, attv_grid=None, pttv_grid=None):
  
  # save chi2r_oc_grid
  chi2r_header_str = 'chi2r_oc {0:s}'.format(' '.join(
    ['chi2r_mean_N{0:d}_d chi2r_median_N{1:d}_d'.format(nT0_sel[i_n], nT0_sel[i_n])
     for i_n in range(len(nT0_sel))
    ]))
  chi2r_header_lst = chi2r_header_str.split()
  
  f_h5.create_dataset('grid/chi2r_oc_grid', data=chi2r_oc_grid,
                      dtype=np.float64, compression="gzip"
                      )
  f_h5.create_dataset('grid/chi2r_header', data=np.array(chi2r_header_lst, dtype='S20'),
                      dtype='S20', compression="gzip"
                      )
  ...
  f_h5.create_dataset('grid/rms_oc_grid', data=rms_oc_grid,
    dtype=np.float64, compression="gzip"
    )
  f_h5.create_dataset('grid/rms_header', data=np.array(rms_header_lst, dtype='S20'),
    dtype='S20', compression="gzip"
    )
  f_h5.create_dataset('grid/above_threshold', data=above_threshold,
    dtype=bool, compression="gzip"
    )
...
def compute_grid_to_chi2r_rms(grid_par, chi2r_all, rms_oc_all, rms_rv=None, of_log=None):
  ...
  for iim in range(0, n_m):
    for iip in range(0, n_p):
      ...
      chi2r_MP.append(np.percentile(np.array(chi2r_temp), 50.0,
        interpolation='midpoint', axis=0)
        )
      
      #rms_oc_MP.append(np.mean(rms_oc_temp, axis=0))
      rms_oc_MP.append(np.percentile(np.array(rms_oc_temp), 50.0,
        interpolation='midpoint', axis=0)
        )
      
      if(rms_rv is not None):
        #rv_MP.append(np.mean(rv_temp, axis=0))
        rv_MP.append(np.percentile(np.array(rv_temp), 50.0,
          interpolation='midpoint', axis=0)
          )
...
def plot_m_vs_p_vs_chi2r(plot_file, mass, period, chi2r_MP, rms_oc_MP, rms_oc_max=5.0/1440.0, K_rv=5., rv_MP=None, P_tra=None, n_label = 'all 3 5 6 10 15'.split()):
  ...
  for iplt in range(0, nplt):
    ...
    ax.contour(mesh_p, mesh_m, mesh_rms_oc, 
      levels=rms_lev, 
      #colors='black',
      colors='white',
      linewidths=0.5,
    ...
    ax.contourf(mesh_p, mesh_m, mesh_rv, 
    ...
    if(P_tra is not None):
      ...
      xlab = r'$P_\textrm{{per}}/P_\textrm{{tra}} , N_{{T0}} = \textrm{{{0:s}}}$'.format(n_label[iplt])
      ...
      xlab = r'$P (\textrm{{days}}) , N_{{T0}} = \textrm{{{0:s}}}$'.format(n_label[iplt])
...
# ==============================================================================
def read_and_plot_p_vs_m_vs_chi2r(out_folder, hdf5_file,\
    rms_oc_max=5.0/1440.0, K_rv=None, P_tra=None, of_log=None):
  ...
  plt_file_mean = os.path.join(out_folder, 'p_vs_m_chi2r_{0:s}_max{1:.2f}min'.format('mean',   rms_oc_max*1440.0))
  plt_file_med  = os.path.join(out_folder, 'p_vs_m_chi2r_{0:s}_max{1:.2f}min'.format('median', rms_oc_max*1440.0))
...
def main():
  ...
  anc.print_both('CHEOPS DATE: LAUNCH = {0:.4f} [BJD] END = {1:.4f} [BJD]'.format(cli.t_start, cli.t_end), of_log)
  ...
  for igrid in range(0, niter):
    ...
    print(' - Saving results for sim #{0:06d} ... into hdf5 file ...'.format(results_sim['sim_num']), end=' ')
    f_h5 = save_results_sim(f_h5, results_sim, cli.mr_type)
  ...
  for imin in [15.0]:
    ...
    _, _ = read_and_plot_p_vs_m_vs_chi2r(out_folder,
                                         hdf5_name,
