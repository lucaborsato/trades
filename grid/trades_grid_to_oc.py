...
plt.rcParams['font.serif'] = [
    'Computer Modern Roman', 'Palatino', 'DejaVu Serif']
...
def set_names_parameters(id_body, mr_unit):
    ...
    units = '{0:s} {1:s} day au - deg deg day deg deg'.format(
        mr_unit[0], mr_unit[1])
    units = units.split()
    n_names = len(base_names_lst)
    names_lst = [r'{0:s}{1:s}'.format(
        base_names_lst[i], body_num) for i in range(0, n_names)]
    temp_lst = [r'{0:s} [{1:s}]'.format(
        names_lst[i], units[i].strip()) for i in range(0, n_names)]
    names_str = ' '.join([r'{0:23s}'.format(temp_lst[i])
...
def simulation_and_ttv_analysis(i_grid, n_bodies, perturber_par, t_start, t_end,
    ...
    try:
        ...
        mass, radius, period, sma, ecc, argp, mean_an, inc, long_n, nt0_full, nrv_nmax = \
            pytrades.wrapper_set_grid_orbelem(
    ...
    if(ttra_full is not None):
        for i in range(1, n_bodies):  # star == 0
            ...
            plot_file = os.path.join(
                plot_folder, 'oc_{0:05d}_NB{1:02d}'.format(sim_num, i_body))
            if(nTTs > 3):
                ...
                try:
                    ...
                    for i_mc in range(n_mc):
                        ...
                        # setting equal to full gls analysis
                        P_MC[i_body][i_mc] = pttv
                except:
                    print(' | PID {0:d} || TID {1:d} sim {2:d}: issue with transits for body {3:d}.'
...
def run_the_simulation(sim_args):
    ...
    pid = os.getpid()
    tid = _thread.get_ident()
    print(' PID {0:d} || TID {1:d} RUNNING sim {2:d} ...'.format(
        pid, tid, i_grid + 1))
    ...
    print(' PID {0:d} || TID {1:d} COMPLETED sim {2:d}'.format(
        pid, tid, i_grid + 1))
...
def ticklabels_logtolin(tickslabels_obj):
    ...
    ticks_log_str = [tickslabels_obj[ii].get_text()
                     for ii in range(len(tickslabels_obj))]
...
def save_full_grid_to_hf5(out_folder, list_results_sim, n_bodies, of_log=None):
    ...
    def dict_to_array_to_h5(d, key, x5, data_type):

        arr = np.column_stack([np.array(k) for k in d.values()])
    ...
    f5 = h5py.File(file_name, "w")

    for r in list_results_sim:
        s_num = "{:07d}".format(r['sim_num'])
        print(r['sim_num'], " ==> ", s_num)
        g5 = f5.create_group(s_num)
        # attributes of the simulation
        # 
        # print("n_bodies = ", n_bodies)
        g5.attrs["n_bodies"] = n_bodies
        ...
        # data
        for k_d in data_keys:
            if isinstance(r[k_d], dict):
                dict_to_array_to_h5(r[k_d],k_d, g5, 'f8')
            # elif isinstance(r[k_d], np.ndarray) or isinstance(r[k_d], list):
            else:
                try:
                    g5.create_dataset(
                        k_d,
                    ...
                    anc.print_both("skipping {}, not dict, list, or numpy.ndarray".format(k_d), of_log)

        g5["keplerian_elements"].attrs["keplerian_elements_header"] = r["keplerian_elements_header"]

        for k, v in r["perturber_par_names"].items():
            g5["perturber_par"].attrs["perturber_par_names_{}".format(k)] = v
            u = r["perturber_par_units"][k]
            g5["perturber_par"].attrs["perturber_par_units_{}".format(k)] = u

        # K_rv
        dict_to_array_to_h5(r["K_rv"],"K_rv", g5, 'f8')
        # A_TTV
        dict_to_array_to_h5(r["A_TTV"],"A_TTV", g5, 'f8')
        # P_TTV
        dict_to_array_to_h5(r["P_TTV"],"P_TTV", g5, 'f8')
        # linear_ephem
        dict_to_array_to_h5(r["linear_ephem"],"linear_ephem", g5, 'f8')
        # statistics
        dict_to_array_to_h5(r["statistics"],"statistics", g5, 'f8')
        g5["statistics"].attrs["cols"] = "chisquare dof red_chisquare"
        # A_TTV_MC
        dict_to_array_to_h5(r["A_TTV_MC"],"A_TTV_MC", g5, 'f8')
        # P_TTV_MC
        dict_to_array_to_h5(r["P_TTV_MC"],"P_TTV_MC", g5, 'f8')

    f5.close()

    anc.print_both("Saved all results in {}".format(file_name), of_log)
...
def main():
    ...
    anc.print_both('TIME SELECTION: START = {0:.4f} (days) END = {1:.4f} (days)'.format(
        cli.t_start, cli.t_end), of_log)
    ...
    if(cli.nthreads <= 1):
        ...
        # if cli.nthreads > mpro.cpu_count() - 1:
        #     nth = mpro.cpu_count() - 1
        # else:
        #     nth = cli.nthreads
        nth = min(cli.nthreads, mpro.cpu_count()-1)
        print(" - === in pool === \n")
        with mpro.Pool(cli.nthreads) as threads_pool:
            list_results_sim = threads_pool.map(
                run_the_simulation, list_sim_args)
    ...
    for i in range(1, n_bodies):
        i_body = i + 1
        s_file = os.path.join(
            out_folder, "summary_NB{:02d}_grid.dat".format(i_body))
        ...
        for i_grid in range(0, niter):
            ...
            summary_grid[i - 1, i_grid, 1] = r["keplerian_elements"][i,
                                                                     0] * mr_convert[0]  # mass
            summary_grid[i - 1, i_grid,
                         2] = r["keplerian_elements"][i, 2]  # period
            summary_grid[i - 1, i_grid,
                         3] = r["perturber_par"][0] * mr_convert[0]  # mass
            summary_grid[i - 1, i_grid, 4] = r["perturber_par"][2]  # period
            summary_grid[i - 1, i_grid,
                         5] = r["linear_ephem"][i_body][0]  # Tref
            summary_grid[i - 1, i_grid,
                         6] = r["linear_ephem"][i_body][1]  # Pref
            summary_grid[i - 1, i_grid, 7] = r["A_TTV"][i_body]  # A_TTV days
            summary_grid[i - 1, i_grid, 8] = r["P_TTV"][i_body]  # P_TTV days
            summary_grid[i - 1, i_grid,
                         9] = r["statistics"][i_body][0]  # chi square
            summary_grid[i - 1, i_grid, 10] = r["statistics"][i_body][1]  # dof
            # red chi square
            summary_grid[i - 1, i_grid, 11] = r["statistics"][i_body][2]
            # A_TTV_MC days
            summary_grid[i - 1, i_grid, 12] = r["A_TTV_MC"][i_body]
            # P_TTV_MC days
            summary_grid[i - 1, i_grid, 13] = r["P_TTV_MC"][i_body]

            if(i_body == pytrades.idpert):
                rv_perturber_grid[i_grid, 0] = r['sim_num']
                # mass
                rv_perturber_grid[i_grid,
                                  1] = r["perturber_par"][0] * mr_convert[0]
    ...
    header_rv = "sim_num mass_perturber_{0:s} period_perturber_d K_rv_mps".format(
        mr_unit[0])
    ...
    save_full_grid_to_hf5(out_folder, list_results_sim, n_bodies)
