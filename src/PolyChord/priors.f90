module priors_module
  ...
  implicit none
    ...
    function sorted_uniform_htp(hypercube_coords,parameters) result(physical_coords)
        ...
        physical_coords(n_prior) = hypercube_coords(n_prior)**(1._dp/n_prior)
        ...
        do n_prior=n_prior-1,1,-1
            physical_coords(n_prior) = hypercube_coords(n_prior)**(1._dp/n_prior)*physical_coords(n_prior+1)
        end do
    ...
    function prior_log_volume(priors) result(log_volume)
        ...
        do i=1,size(priors)
            ...
            case(gaussian_type)
                log_volume = log_volume + sum( 0.5_dp*logTwoPi + log(priors(i)%parameters(2::2)) )
            case(log_uniform_type)
                log_volume = log_volume + sum( log(log( priors(i)%parameters(2::2)/priors(i)%parameters(1::2) )) ) 
            case(sorted_uniform_type)
                log_volume = log_volume + log(priors(i)%parameters(2)- priors(i)%parameters(1) ) - log(gamma(1._dp+priors(i)%npars)) 
            end select
        end do
