...
module read_write_module
  use constants,only:dp
    ...
    subroutine read_resume_file(settings,RTI)
        ...
        real(dp), dimension(settings%nTotal) :: temp_point
        real(dp), dimension(settings%nposterior) :: temp_posterior
        real(dp), dimension(settings%np) :: temp_equal
    ...
    subroutine write_posterior_file(settings,RTI)
        ...
        if(settings%equals) then
            ...
            do i_post=1,RTI%nequals_global(1)
                write(write_equals_unit,fmt_dbl) 1._dp,RTI%equals_global(settings%p_2l:,i_post,1)
            end do
        ...
        if(settings%posteriors) then
            ...
            do i_post=1,RTI%nposterior_global(1)
                ...
                if( weight>0._dp  ) write(write_posterior_unit,fmt_dbl) weight,-2._dp*RTI%posterior_global(settings%pos_l,i_post,1),RTI%posterior_global(settings%pos_p0:,i_post,1) 
            end do
                ...
                do i_cluster = 1,RTI%ncluster+RTI%ncluster_dead
                    ...
                    if(ordering(i_cluster)<=RTI%ncluster) then
                        ...
                        do i_post = 1,RTI%nposterior(ordering(i_cluster))
                            ...
                            if( weight>0._dp  ) write(write_posterior_unit,fmt_dbl) weight,-2._dp*RTI%posterior(settings%pos_l,i_post,ordering(i_cluster)),RTI%posterior(settings%pos_p0:,i_post,ordering(i_cluster)) 
                        end do
                    else
                        ...
                        do i_post = 1,RTI%nposterior_dead(ordering(i_cluster)-RTI%ncluster)
                            ...
                            if( weight>0._dp  ) write(write_posterior_unit,fmt_dbl) weight,-2._dp*RTI%posterior_dead(settings%pos_l,i_post,ordering(i_cluster)-RTI%ncluster),RTI%posterior_dead(settings%pos_p0:,i_post,ordering(i_cluster)-RTI%ncluster) 
                        end do
    ...
    subroutine write_stats_file(settings,RTI,nlikesum)
        ...
        write(write_stats_unit,fmt_nlike) real(nlikesum,dp)/real(settings%update_resume,dp),real(nlikesum,dp)/real(RTI%num_repeats*settings%update_resume,dp)
