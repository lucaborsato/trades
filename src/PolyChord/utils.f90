module utils_module
  use constants,only:dp
    ...
    real(dp), parameter :: logzero = -sqrt(huge(0._dp))
    !> The effective value of \f$ log(\inf) \f$
    real(dp), parameter :: loginf = +sqrt(huge(0._dp))
    ...
    real(dp), parameter :: logTwoPi = log(8._dp*atan(1._dp))
...
!     parameter( eps = 1d-15 )
    ...
    function MP(a,b,live_data)
        ...
        do i=1,size(live_data,2)
            if(distance2(live_data(:,i),a) > dab2 .and. distance2(live_data(:,i),b) > dab2 ) MP = MP+1._dp
        end do

        MP = MP/(size(live_data,2)+0._dp)
    ...
    function MP2(seed,baby,live_data)
        ...
        do i=1,size(live_data,2)
            if(distance2(live_data(:,i),seed) > dab2 ) MP2 = MP2+1._dp
        end do

        MP2 = MP2/(size(live_data,2)+0._dp)
    ...
    function identity_matrix(nDims)
        ...
        do i_dims=1,nDims
            identity_matrix(i_dims,i_dims) = 1._dp
        end do
    ...
    function Hypergeometric1F1(a,b,z)
        ...
        Hypergeometric1F1=0._dp
        n=0

        do while( abovetol(change,Hypergeometric1F1) )
            change = Pochhammer(a,n) * Pochhammer(b,n) * z**n / gamma(1._dp+n)
    ...
    function Hypergeometric2F1(a,b,c,z)
        ...
        Hypergeometric2F1=0._dp
        n=0

        do while( abovetol(change,Hypergeometric2F1) )
            change = Pochhammer(a,n) * Pochhammer(b,n) / Pochhammer(c,n) * z**n / gamma(1._dp+n)
    ...
    function calc_similarity_matrix(data_array) result(similarity_matrix)
        ...
        similarity_matrix = similarity_matrix + transpose(similarity_matrix) - 2._dp * matmul( transpose(data_array),data_array )
    ...
    function Vn(nDims)
        ...
        integer,intent(in) :: nDims
        real(dp) :: Vn
        real(dp), parameter :: sqrtpi = sqrt(4._dp*atan(1._dp))
        Vn = sqrtpi**nDims /gamma(1._dp+nDims/2._dp)
    end function Vn
    ...
    function normal_cdf(x)
        ...
        normal_cdf = 0.5_dp * ( 1._dp + erf(x/sqrt2))
...
!         log_gauss = - ( size(theta) * logTwoPi + logdetcovmat )/2d0
        log_gauss = - ( size(theta) * logTwoPi + logdetcovmat )*0.5_dp

!         log_gauss = log_gauss - dot_product(theta-mean,matmul(invcovmat,theta-mean))/2d0
        log_gauss = log_gauss - dot_product(theta-mean,matmul(invcovmat,theta-mean))*0.5_dp
