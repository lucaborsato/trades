module run_time_module
  ...
  implicit none
    ...
    function update_evidence(RTI,p) result(logweight)
        ...
        real(dp),parameter :: log2 = log(2._dp)
        real(dp) :: lognp
        real(dp) :: lognp1
        real(dp) :: lognp2
        ...
        lognp = log( RTI%nlive(p) + 0._dp )
        lognp1= log( RTI%nlive(p) + 1._dp )
        lognp2= log( RTI%nlive(p) + 2._dp )
        ...
        call logincexp( RTI%logZ2 ,                             &
            ...
            log2 + RTI%logXpXq(p,p)  + 2._dp*logL - lognp1 - lognp2 &
            )
    ...
    subroutine add_cluster(settings,RTI,p,cluster_list,num_new_clusters) 
        ...
        logni  = log(RTI%nlive(new_target) + RTI%nphantom(new_target) + 0._dp)
        logni1 = log(RTI%nlive(new_target) + RTI%nphantom(new_target) + 1._dp)
        logn   = logsumexp(logni)
        logn1  = logaddexp(logn,0._dp)
    ...
    subroutine calculate_logZ_estimate(RTI,logZ,varlogZ,logZp,varlogZp,logZp_dead,varlogZp_dead)
        ...
        if(present(logZp).and.present(varlogZp))then
            logZp      = max(logzero,2._dp*RTI%logZp - 0.5_dp*RTI%logZp2)
            varlogZp   = RTI%logZp2 - 2._dp*RTI%logZp
        end if

        if(present(logZp_dead).and.present(varlogZp_dead))then
            logZp_dead      = max(logzero,2._dp*RTI%logZp_dead - 0.5_dp*RTI%logZp2_dead)
            varlogZp_dead   = RTI%logZp2_dead - 2._dp*RTI%logZp_dead
        end if
    ...
    subroutine update_posteriors(settings,RTI)
        ...
        if(settings%equals) then
            ...
            ! LUCA BORSATO
            write(*,*)" cleaned locally "
            flush(6)
        end if
        
        ! LUCA BORSATO
        write(*,'(a)')" Adding new posterior points "
        flush(6)
