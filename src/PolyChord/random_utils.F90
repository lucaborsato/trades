...
module random_module
  use constants,only:sp,dp
    ...
    function basic_random(seed)
        ...
        basic_random = 1._sp * oldseed / real(c2,sp)
    ...
    function random_direction(nDims)
        ...
        random_direction2=0._dp
        do while(random_direction2<=0._dp)
    ...
    function random_subdirection(nDims,rhat)
        ...
        random_subdirection2=0._dp
        do while (random_subdirection2<=0._dp)
    ...
    function random_point_in_sphere(nDims)
        ...
        rand_rad = random_real()**(1._dp/nDims)
    ...
    function random_distinct_integers(m,k) result(integers)
        implicit none
        real(dp), parameter :: cutoff=0.6321205588285576784_dp
    ...
    subroutine random_inverse_covmat(invcovmat,logdetcovmat,sigma,nDims)
        ...
        do j=1,nDims
            eigenvalues(j)  = sigma * rng**((j-1._dp)/(nDims-1._dp))
        end do
        ...
        invcovmat = 0._dp
        do j=1,nDims
            invcovmat(j,j) = 1._dp/eigenvalues(j)**2
        end do
        ...
        logdetcovmat = 2._dp * sum(log(eigenvalues))
