  ...
  use constants,only:dp
    ...
    subroutine write_generating_live_points(feedback,i_live,nlive)
        ...
        if (feedback>=fancy_fb) then
            call progress(real(i_live,dp)/real(nlive,dp),100)
        end if
    ...
    subroutine write_intermediate_results(settings,RTI,nlikesum)
        ...
        int_width = ceiling(log10(real(maxval([RTI%nlive(:RTI%ncluster),RTI%nphantom(:RTI%ncluster),RTI%nposterior(:RTI%ncluster),RTI%nequals(:RTI%ncluster)]),dp)))
...
!             write(stdout_unit,fmt_nlike) dble(nlikesum)/dble(settings%nlive),dble(nlikesum)/dble(RTI%num_repeats*settings%nlive)
            write(stdout_unit,fmt_nlike) real(nlikesum,dp)/real(settings%nlive,dp),real(nlikesum,dp)/real(RTI%num_repeats*settings%nlive,dp)
