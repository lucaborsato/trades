  ...
  implicit none
    ...
    function SliceSampling(loglikelihood,priors,settings,logL,seed_point,cholesky,nlike,num_repeats)  result(baby_points)
        ...
        do i_babies=1,size(nhats,2)
            ...
            ! LUCA BORSATO
            if(dot_product(nhat,nhat).le.0._dp)then
              write(*,*)" -------SliceSampling--------- "
              write(*,*)" i_babies = ",i_babies
              write(*,'(a,f15.8)')" dot_product(nhat,nhat) = ",dot_product(nhat,nhat)
              write(*,*)" ----------------------------- "
              flush(6)
              stop
            end if
            ...
            ! LUCA BORSATO
            if(abs(w).le.epsilon(0._dp))then
              write(*,*)" -------SliceSampling--------- "
              write(*,*)" i_babies = ",i_babies
              write(*,*)" abs(w) <= epsilon(0._dp) == ",&
                &abs(w)," <= ",epsilon(0._dp)
              write(*,*)" ----------------------------- "
              flush(6)
              stop
            ...
            nhat = nhat/w
            w = w * 3._dp !* exp( lgamma(0.5d0 * settings%nDims) - lgamma(1.5d0 + 0.5d0 * settings%nDims) ) * settings%nDims
    ...
    function slice_sample(loglikelihood,priors,logL,nhat,x0,w,S,n) result(baby_point)
        ...
        recursive function find_positive_within(L,R) result(x1)
            ...
            if( x1(S%l0) < logL .or. x1(S%l0) <= logzero ) then
                ...
                if ( dot_product(x1(S%h0:S%h1)-x0(S%h0:S%h1),nhat) > 0._dp ) then
