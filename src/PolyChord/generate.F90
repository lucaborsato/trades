  ...
  use constants,only:dp
    ...
    subroutine GenerateLivePoints(loglikelihood,priors,settings,RTI,mpi_information)
        ...
        if(is_root(mpi_information)) then
            ...
            RTI%num_repeats(2:) = nint(settings%grade_frac(2:)/(settings%grade_frac(1)+0._dp)*RTI%num_repeats(1)*speed(1)/speed(2:))

            ! Set the posterior thinning factor
            if(settings%boost_posterior<0._dp) then
                RTI%thin_posterior = 1._dp
            else
                RTI%thin_posterior = (settings%boost_posterior+0._dp)/(sum(RTI%num_repeats)+0._dp)
            end if
    ...
    subroutine time_speeds(loglikelihood,priors,settings,RTI,speed,mpi_information)
        ...
        do i_speed=2,size(speed)
            ...
            do while( total_time/settings%grade_frac(i_speed)< speed(1)/settings%grade_frac(1) *settings%nlive / real(mpi_information%nprocs ,dp))
