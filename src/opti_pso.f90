...
!     integer::ii
  ...
  subroutine do_pso(n, xmin, xmax, xinit, np, it, ir, maximize, evfunc, xall, x, r, iGlobal)
    ...
    interface
      !-------- evaluation function -------
      integer function evfunc(xall, x, r)  ! return number of constrained condition
        use constants,only:dp
        real(dp),dimension(:),intent(in)::xall,x    ! parameter set
        real(dp), intent(out) :: r       ! evaluation value
      end function evfunc
    end interface
...
!       if(i.eq.1)stop
  ...
  subroutine p_move(n, gxbest, w, c1, c2, c3, vmax, vrand, flag, p)
    ...
    real(dp),intent(in):: vrand         ! velocity parametebtion parameter
    integer,      intent(in)    :: flag          ! control flag
...
!           write(*,*)' par (True) = ',unscaling(nfit,xtemp)
  ...
  function evaluate(n, xall, x, sign, evfunc, evcount) result(r)
    ...
    real(dp)                   :: r
    interface
      integer function evfunc(xall, x, r)
        use constants,only:dp
        real(dp),dimension(:),intent(in)::xall,x
