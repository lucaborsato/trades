...
!     write(*,'(a)')trim(adjustl(all_names_str))
    ...
    subroutine idpar()
        ...
        do j = 1, nkep
            ...
            ! write(*,*)j,trim(parid(j))

        end do ! end do j=1,nkep

        ! write(*,*)"outside j = ",j
        j = nkep
        ! write(*,*)"reset j =nkep = ",j
                    ...
                    if (id(ifit) .eq. 3) then
                        ...
                        if (ifit .lt. nfit) then
                            if (id(ifit + 1) .eq. 10) then
                                minpar(ifit) = -180.0_dp
                                minpar(ifit + 1) = -180.0_dp
                                maxpar(ifit) = +180.0_dp
                                maxpar(ifit + 1) = +180.0_dp
                                done(ifit) = .true.
                                done(ifit + 1) = .true.
                            else
...
!     call set_minmax()
    ...
    subroutine par2kel_fit(all_parameters, fit_parameters,&
        ...
        &checkpar)
        use celestial_mechanics, only: get_semax
...
!     checkpar=.true.
        ...
        do i_body = 2, NB
            ...
            if (tofit(i_par) .eq. 1) temp_kel(1) = atemp(i_par)*MR_star(1, 1) ! mp/Ms to mp

            if (tofit(i_par + 3) .eq. 1 .and. tofit(i_par + 4) .eq. 1) then ! sqrt(e)cosw,sqrt(e)sinw to e,w
                ! temp2 = sqrt((atemp(i_par + 3)*atemp(i_par + 3)) + (atemp(i_par + 4)*atemp(i_par + 4))) ! OLD: ecosw, esinw ==> e,w
...
!           return
                ...
                if (temp_kel(4) .le. TOL_dp) temp_kel(5) = 90.0_dp
                if (abs(temp_kel(4) - atemp(i_par + 3)) .le. TOL_dp) then
                    if (abs(atemp(i_par + 4)) .gt. TOL_dp) checkpar = .false.
                else if (abs(temp_kel(4) - atemp(i_par + 4)) .le. TOL_dp) then
                    if (abs(atemp(i_par + 3)) .gt. TOL_dp) checkpar = .false.
                end if
            end if

            if (tofit(i_par + 6) .eq. 1) then !inc fit
                if (tofit(i_par + 7) .eq. 1) then
                    temp_kel(7) = sqrt(atemp(i_par + 6)*atemp(i_par + 6) + atemp(i_par + 7)*atemp(i_par + 7)) ! icoslN,isinlN to inc,lN
...
!           return
            ...
            if (tofit(i_par + 5) .eq. 1) temp_kel(6) = mod(mod((atemp(i_par + 5) - temp_kel(5) - temp_kel(8)), circ) + circ, circ) ! lambda to mA
