...
!     deallocate(rin,drdt,err)
    ...
    subroutine find_transit(itra, m, r1, r2, itime, hok, tmidtra, lte, ro, status)
        integer, intent(in)::itra
            ...
            if (r_sky .le. Rmax) then ! planet transits the star (b <= 1)
                ...
                if (do_transit(id_body)) then ! planet has to transit!
                    ...
                    if (obsData%obsT0(id_body - 1)%nT0 .gt. 0) &
                      &call assign_T0_byNumber(id_body, mass, rtra, obsData, tmidtra, duration, simT0)
        ...
        if (Hc) then
            ...
            if (r_sky .le. Rmax) then ! planet transits the star (b <= 1)
                ...
                if (transit_flag(id_body)) then ! planet has to transit!
                    ...
                    if (oDataIn%obsT0(id_body - 1)%nT0 .gt. 0) &
    ...
    subroutine assign_T0_byNumber(id_body, mass, rtra, oDataIn, tmidtra, duration, simT0)
        ...
        do in = 1, nTs
            ...
            if (ntmid .eq. epo) then
                ...
                ! NOT CHECKING IF dur_check/durcheck is 1, simply assign it
                ! if (dur_check .eq. 1) then
                ...
                call elements_one_body(id_body, mass, rtra,&
                    &simT0(ibd)%period(in), simT0(ibd)%sma(in),&
                    &simT0(ibd)%ecc(in), simT0(ibd)%inc(in), simT0(ibd)%meanA(in),&
                    &simT0(ibd)%argp(in), simT0(ibd)%trueA(in),&
                    &simT0(ibd)%longN(in), simT0(ibd)%dttau(in))
            end if
        end do
...
!     rtra=one
        rtra = r1
        call find_transit(id_body, mass, r1, r2, itime, hok, ttra, lte, rtra, status)
        ...
        if (status) then
            ...
            ! else
            !     if(id_body .eq. 5) write(*,*)" all_transits: r_sky > Rmax!"
            end if ! r_sky

        ! else 
        !     if(id_body .eq. 5) write(*,*)" all_transits: status is false!"
        end if ! status
