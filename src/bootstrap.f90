...
!     real(dp),dimension(:,:),allocatable::T0ok,T0boot
        ...
        real(dp)::chi_square, reduced_chi_square, lnLikelihood, ln_const, bic, scale_errorbar
...
!     call ode_parok(allpar,parok,RVok,T0ok,resw,fitness)
        call ode_parok(allpar, parok, okRV, okT0, resw,&
        ...
        scale_errorbar = one
        if (bootstrap_scaling) scale_errorbar = sqrt(reduced_chi_square)
...
!$      cpuid = omp_get_thread_num()+1
        ...
        write (uboot, '(4(a,es23.16),a)') '# Mstar=N(', MR_star(1, 1), ' , ',&
            &MR_star(1, 2), ') Rstar=N(', MR_star(2, 1), ' , ', MR_star(2, 2), ') '
!     write(*,*)" writing ",trim(flboot)
        do ip = 1, nboot
